...
(define (expmod-28-b base exp m under-test)
  ...
  (define (iter base exp m)
    (cond ((= exp 0) 1)
	  ((even? exp)
	   (let ((r (remainder (square (iter base (/ exp 2) m))
			       m)))
	     (println base exp m)
	     (if (nontrivial? r exp) 0 r)))
	  (else
	   (let ((r (remainder (* base (iter base (- exp 1) m))
			       m)))
	     (println base exp m)
	     (if (nontrivial? r exp) 0 r)))))
  (iter base exp m))

(define (expmod-28 base exp m n)
  (define (nontrivial? r a)
    (and (= r 1)
	 (not (= a 1))
	 (not (= a (- n 1)))))
  (define (iter base exp m)
    ...
    (cond ((= exp 0) 1)
	  ((even? exp)
	   (remainder (square (iter base (/ exp 2) m))
		      m))
	  (else
	   (remainder (* base (iter base (- exp 1) m))
		      m))))
  (iter base exp m))

;; (expmod 7 43 43)
;; (expmod 12 42 43)
;; (expmod-28 7 42 43 43)
;; (expmod-28 12 42 42)

;; (expmod-28 56 561 561)
;; (expmod 56 561 561)

(define (fast-prime-28? under-test times)
  (define (fermat-test n)
    (define (try-it a)
      (= 1 (expmod-28-b a (- n 1) n under-test)))
    (try-it (+ 1 (random (- n 1)))))
  (cond ((= times 0) true)
        ((fermat-test under-test) (fast-prime? under-test (- times 1)))
        (else false)))

(fast-prime? 561 10)
(fast-prime-28? 561 10)
(fast-prime-28? 1105 5)
(fast-prime-28? 43 10)
(fast-prime? 43 2)

;; This still fails part of the time and I don't think it should. Also the lets
;; seem sloppy and not quite what I want.
